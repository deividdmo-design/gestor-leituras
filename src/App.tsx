    ...
    <div className="min-h-screen bg-[#F9F7F2] text-slate-900 font-sans tracking-tight">
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-white/40 backdrop-blur-sm p-5 rounded-3xl border border-white/60 shadow-sm flex flex-col items-center"><Trophy className="text-amber-600 mb-1"/><p className="text-xl font-black">{stats.totalReadPages.toLocaleString()}</p><p className="text-[8px] text-stone-400 font-black uppercase tracking-widest">Páginas</p></div>
          ...
          <div className="bg-white/40 backdrop-blur-sm p-5 rounded-3xl border border-white/60 shadow-sm flex flex-col items-center"><Star className="text-amber-600 mb-1"/><p className="text-xl font-black">7.4</p><p className="text-[8px] text-stone-400 font-black uppercase tracking-widest">Média</p></div>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            <div className="bg-white/40 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/60 shadow-xl">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-2xl border border-stone-100"><span className="text-[10px] font-black uppercase text-stone-400">Objetivo:</span><input type="number" value={readingGoal} onChange={e => setReadingGoal(Number(e.target.value))} className="w-12 bg-transparent font-black text-stone-900 outline-none"/></div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white/40 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/60 shadow-sm">
                ...
                <div className="space-y-6">{analytics.topGeneros.map(([n, c]) => (<div key={n}><div className="flex justify-between items-center mb-2 font-black text-[10px] uppercase"><span>{n}</span><span className="text-stone-400">{c} Livros</span></div><div className="w-full bg-stone-950/5 h-2 rounded-full overflow-hidden shadow-inner"><div className={`h-full transition-all duration-1000 ${genreBarColors[n] || 'bg-stone-900'}`} style={{ width: `${(c / (stats.totalBooks || 1)) * 100}%` }}></div></div></div>))}</div>
              </div>
              <div className="bg-white/40 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/60 shadow-sm">
      ...
      {isShuffleOpen && shuffledBook && (
        <div className="fixed inset-0 bg-stone-950/95 backdrop-blur-2xl flex items-center justify-center z-50 p-4 animate-in fade-in duration-500">
          <div className="bg-white/5 w-full max-w-lg rounded-[4rem] p-12 text-center relative overflow-hidden border border-white/10">
