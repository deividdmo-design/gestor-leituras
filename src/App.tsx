...
// CORES DOS GÊNEROS (MAPEADAS PELAS CATEGORIAS PRINCIPAIS)
const genreColors: Record<string, string> = {
  'Ficção': 'bg-fuchsia-50 text-fuchsia-700 border-fuchsia-100',
  'Não Ficção': 'bg-slate-50 text-slate-700 border-slate-100',
  ...
  'Ciências da Saúde': 'bg-rose-50 text-rose-700 border-rose-100',
...
export default function App() {
  ...
  const analytics = useMemo(() => {
    ...
    books.forEach(b => {
      ...
      if (b.status === 'Concluído' && b.finished_at && new Date(b.finished_at).getFullYear() === currentYear) {
        counters.monthly[new Date(b.finished_at).getMonth()]++;
      }
    });
    return { formatos: counters.formats, mensal: counters.monthly, topAuthors: Object.entries(counters.authors).sort((a,b) => b[1] - a[1]).slice(0,3), topCountries: Object.entries(counters.countries).sort((a,b) => b[1] - a[1]).slice(0,3) };
  }, [books]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      {/* ✅ MODAL DE CADASTRO ATUALIZADO COM OS NOVOS GÊNEROS */}
      {isModalOpen && (
