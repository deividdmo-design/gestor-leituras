        ...
        {currentView === 'library' ? (
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm"><h2 className="text-sm font-black uppercase text-slate-900 mb-6 flex items-center gap-2"><Monitor className="w-4 h-4 text-orange-600"/> Formatos</h2><div className="space-y-6">{analytics.genres.slice(0, 4).map(([name, count]) => (<div key={name}><div className="flex justify-between text-[10px] font-black uppercase mb-1"><span>{name}</span><span>{Math.round((count / books.length) * 100)}%</span></div><div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden"><div className="bg-slate-800 h-full rounded-full" style={{ width: `${(count / books.length) * 100}%` }}></div></div></div>))}</div></div>
            </div>
          </div>
        )}
      </main>
