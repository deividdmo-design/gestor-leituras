...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
          ...
          <>
            ...
            <div className="bg-white p-2 rounded-[1.5rem] border border-slate-200 shadow-sm flex flex-col lg:flex-row gap-2">
                ...
                <div className="flex items-center gap-2 px-2">
                    {['Todos', 'Na Fila', 'Lendo', 'Concluído', 'Abandonado'].map((s) => (<button key={s} onClick={() => setFilterStatus(s)} className={`px-4 py-2.5 rounded-xl text-xs font-bold uppercase ${filterStatus === s ? 'bg-slate-900 text-white' : 'text-slate-500'}`}>{s}</button>))}
                        ...
                        <div key={book.id} className="bg-white p-5 rounded-[2rem] border border-slate-100 flex gap-6 relative overflow-hidden group shadow-sm hover:shadow-md transition-all">
                            ...
                            <p className="text-sm text-slate-500 flex items-center gap-1 mt-1">
                                {(() => {
                                    ...
                                    return countryFlags[nat] ? <span title={book.author_nationality}>{countryFlags[nat]}</span> : <Globe className="w-3 h-3" />;
                                })()}
                                {book.author}
                            </p>
                            <div className="flex flex-wrap gap-2 mt-4 items-center">
                                <span className={`text-[10px] font-black px-2 py-1 rounded uppercase ${book.status === 'Concluído' ? 'bg-emerald-50 text-emerald-600' : book.status === 'Abandonado' ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-600'}`}>{book.status}</span>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                    ...
                    {/* AQUI ESTAVA O ERRO: AGORA USA ANALYTICS.FORMATOS */}
      ...
      {/* MODAL COM CAMPO DE GÊNERO REINTRODUZIDO */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
           <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl p-8 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-black text-slate-900">{editingBookId ? 'Editar BI' : 'Novo Livro'}</h2><button onClick={() => setIsModalOpen(false)} className="p-2 bg-slate-50 rounded-full"><X className="w-5 h-5"/></button></div>
              <form onSubmit={handleSubmit} className="space-y-4">
                 ...
                 {/* CAMPO DE GÊNERO RECOLOCADO AQUI */}
                 <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 ml-1 flex items-center gap-1"><Tag className="w-3 h-3"/> Gênero</label>
                    <select className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold appearance-none outline-none" value={formData.genre} onChange={e => setFormData({...formData, genre: e.target.value})}>
                        <optgroup label="Gestão & Negócios"><option>Gestão</option><option>Estratégia</option><option>Liderança</option><option>Vendas</option><option>Marketing</option><option>RH</option><option>Processos</option><option>Startups</option><option>Finanças</option><option>Negociação</option></optgroup>
                        <optgroup label="Técnico & Tech"><option>Tecnologia</option><option>Data Science</option></optgroup>
                        <optgroup label="Literatura"><option>Ficção</option><option>Romance</option><option>Fantasia</option><option>Sci-Fi</option><option>Clássicos</option></optgroup>
                        <optgroup label="Outros"><option>História</option><option>Biografia</option><option>Outros</option></optgroup>
                    </select>
                 </div>
