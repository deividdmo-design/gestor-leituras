    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-100"><Trophy className="text-blue-600 mb-2"/><p className="text-2xl font-black">{stats.totalReadPages.toLocaleString()}</p><p className="text-xs text-slate-400 font-bold">Páginas</p></div>
        ...
        {currentView === 'library' ? (
              ...
              {books.filter(b => (b.title.toLowerCase().includes(searchTerm.toLowerCase())) && (filterStatus === 'Todos' || b.status === filterStatus)).map(book => (
                <div key={book.id} className="bg-white p-5 rounded-[2rem] border border-slate-100 flex gap-6 relative group shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="flex-1 py-1">
                    ...
                    <div className="flex flex-wrap gap-2 mt-4 items-center"><span className={`text-[10px] font-black px-2 py-1 rounded uppercase ${book.status === 'Concluído' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>{book.status}</span></div>
                  </div>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm"><h2 className="text-sm font-black uppercase mb-6 flex items-center gap-2"><User size={16} className="text-blue-600"/> Autores</h2>{analytics.topAuthors.map(([n, c]) => (<div key={n} className="flex justify-between items-center mb-4 font-bold text-sm"><span>{n}</span><span className="bg-slate-50 px-2 py-1 rounded">{c}</span></div>))}</div>
              <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm"><h2 className="text-sm font-black uppercase mb-6 flex items-center gap-2"><Globe size={16} className="text-emerald-600"/> Países</h2>{analytics.topCountries.map(([n, c]) => (<div key={n} className="flex justify-between items-center mb-4 font-medium text-sm"><span>{countryFlags[n] || '🏳️'} {n.toUpperCase()}</span><span className="text-slate-400 font-bold">{c}</span></div>))}</div>
              <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm"><h2 className="text-sm font-black uppercase mb-6 flex items-center gap-2"><Monitor size={16} className="text-orange-600"/> Formatos</h2>{Object.entries(analytics.formatos).map(([n, c]) => (<div key={n} className="mb-4"><div className="flex justify-between text-[10px] font-black uppercase mb-1"><span>{n}</span><span>{c}</span></div><div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden"><div className="bg-slate-800 h-full" style={{ width: `${(c / books.length) * 100}%` }}></div></div></div>))}</div>
            </div>
          </div>
        )}
      </main>
