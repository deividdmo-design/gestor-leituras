...
export default function App() {
  ...
  async function startReadingShuffled() {
    ...
    try {
      const { error } = await supabase.from('books').update({ status: 'Lendo', started_at: new Date().toISOString().split('T')[0] }).eq('id', shuffledBook.id);
  ...
  const analytics = useMemo(() => {
    const counters = { authors: {} as Record<string, number>, countries: {} as Record<string, number>, genres: {} as Record<string, number>, formats: {} as Record<string, number>, monthly: Array(12).fill(0) };
        ...
        {currentView === 'library' ? (
              ...
              {books.filter(b => (b.title.toLowerCase().includes(searchTerm.toLowerCase()) || b.author.toLowerCase().includes(searchTerm.toLowerCase())) && (filterStatus === 'Todos' || b.status === filterStatus)).map(book => (
                <div key={book.id} className="bg-white p-5 rounded-[2rem] border border-slate-100 flex gap-6 relative group shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="flex-1 py-1 min-w-0">
                    {/* ✅ Proteção contra undefined index e cores dinâmicas */}
                    ...
                    <div className="mt-4"><div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden"><div className="h-full bg-blue-600 rounded-full transition-all duration-500" style={{ width: `${Math.round(((book.read_pages || 0) / (book.total_pages || 1)) * 100)}%` }}></div></div></div>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-6 rounded-3xl border shadow-sm flex flex-col justify-center"><p className="text-xs font-bold text-slate-400 uppercase mb-1">Top País</p><p className="text-xl font-black text-slate-900 uppercase truncate">{analytics.topPaises[0]?.[0] || '-'}</p></div>
              <div className="bg-white p-6 rounded-3xl border shadow-sm flex flex-col justify-center"><p className="text-xs font-bold text-slate-400 uppercase mb-1">Top Gênero</p><p className="text-xl font-black text-slate-900 uppercase truncate">{analytics.topGeneros[0]?.[0] || '-'}</p></div>
            </div>
