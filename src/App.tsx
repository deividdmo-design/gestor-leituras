...
import { 
  ...
  Book, Award, PieChart, LayoutGrid, Monitor, Tag, Shuffle, Sparkle, Loader2
} from 'lucide-react'
...
export default function App() {
  ...
  // 🎲 SORTEADOR COM ANIMAÇÃO DE ROLETA
  async function handleShuffle() {
    ...
    // Efeito de roleta (trocando capas aleatoriamente por 2 segundos)
    let counter = 0;
    const interval = setInterval(() => {
      ...
      if (counter > 15) {
        ...
        const finalSelection = queue[Math.floor(Math.random() * queue.length)];
        setShuffledBook(finalSelection);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setFormData(emptyForm); setIsModalOpen(false); refreshBooks(); alert('✅ Salvo no Acervo.');
  ...
  return (
    <div className="min-h-screen bg-[#F9F7F2] text-slate-900 font-sans tracking-tight">
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        {currentView === 'library' ? (
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            {/* Relatórios permanecem com design executivo */}
      ...
      {/* 🎲 NOVO SORTEADOR CINEMATIC ROLETTE */}
      {isShuffleOpen && shuffledBook && (
        <div className="fixed inset-0 bg-stone-950/95 backdrop-blur-2xl flex items-center justify-center z-50 p-4 animate-in fade-in duration-500">
          <div className="bg-white/5 w-full max-w-lg rounded-[4rem] p-12 text-center relative overflow-hidden border border-white/10 shadow-[0_0_100px_rgba(217,119,6,0.1)]">
            ...
            <div className="relative z-10 space-y-8">
              ...
              <div className="relative group">
                <div className={`w-52 h-80 bg-stone-900 rounded-[2.5rem] mx-auto shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden border-[6px] transition-all duration-700 ${isShuffling ? 'border-white/5 scale-95 blur-[2px]' : 'border-white/20 scale-105 shadow-amber-600/20'}`}>
                  {shuffledBook.cover_url && <img src={shuffledBook.cover_url} className="w-full h-full object-cover" alt="Sorteio" />}
                </div>
                {!isShuffling && <div className="absolute -inset-4 bg-amber-600/10 blur-3xl -z-10 animate-pulse"></div>}
              </div>

              <div className={`transition-all duration-700 transform ${isShuffling ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
                <h2 className="text-[11px] font-black uppercase tracking-[0.5em] text-amber-500/60 mb-2">Destino Literário</h2>
                <h3 className="text-3xl font-black text-white mb-2 leading-tight drop-shadow-lg">{shuffledBook.title}</h3>
                <p className="text-sm font-bold text-stone-400 uppercase tracking-widest">{shuffledBook.author}</p>
                
                <div className="flex gap-4 mt-10">
                  <button onClick={startReadingShuffled} className="flex-1 bg-white text-stone-950 py-5 rounded-2xl font-black uppercase tracking-widest shadow-2xl hover:bg-amber-500 hover:text-white transition-all transform hover:-translate-y-1">Aceitar Desafio</button>
                  <button onClick={handleShuffle} className="px-8 bg-white/5 text-white py-5 rounded-2xl font-black uppercase tracking-widest border border-white/10 hover:bg-white/10 transition-all">Girar Novamente</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* MODAL DE CADASTRO (DESIGN EXECUTIVO) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-stone-950/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] p-8 max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center mb-8 border-b border-stone-50 pb-6"><h2 className="text-lg font-black text-stone-900 uppercase tracking-[0.2em]">{editingBookId ? 'Editar' : 'Novo Acervo'}</h2><button onClick={() => setIsModalOpen(false)} className="p-2 bg-stone-50 rounded-full text-stone-400 hover:text-stone-900"><X/></button></div>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="flex gap-2"><input className="flex-1 bg-stone-50 rounded-2xl px-6 py-4 font-bold outline-none border-2 border-transparent focus:border-stone-200" placeholder="Título" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} required/><button type="button" onClick={searchGoogleBooks} className="bg-stone-900 text-amber-500 px-5 rounded-2xl hover:bg-stone-800 shadow-lg"><Sparkles/></button></div>
              ...
              <div className="space-y-1"><label className="text-[10px] font-black text-stone-400 ml-2 uppercase tracking-widest"><ImageIcon size={12} className="inline mr-1"/> Capa (Manual)</label><div className="relative"><LinkIcon className="absolute left-5 top-1/2 -translate-y-1/2 text-stone-300 w-4 h-4"/><input className="w-full bg-stone-50 rounded-2xl pl-12 pr-4 py-4 text-[11px] font-bold outline-none border-2 border-transparent focus:border-stone-200" placeholder="URL da imagem..." value={formData.cover_url} onChange={e => setFormData({ ...formData, cover_url: e.target.value })}/></div></div>
              <div className="space-y-1"><label className="text-[10px] font-black text-stone-400 ml-2 uppercase tracking-widest"><Tag size={12} className="inline mr-1"/> Classificação</label>
                <select className="w-full bg-stone-50 rounded-2xl px-6 py-4 text-sm font-bold outline-none appearance-none cursor-pointer border-2 border-transparent focus:border-stone-200" value={formData.genre} onChange={e => setFormData({...formData, genre: e.target.value})}>
                  <optgroup label="Categorias"><option>Ficção</option><option>Literatura Clássica</option><option>Não Ficção</option><option>Ciências Humanas</option><option>Tecnologia & Computação</option><option>Outros</option></optgroup>
                </select>
              </div>
              ...
              <button type="submit" className="w-full bg-stone-950 text-white py-5 rounded-2xl font-black uppercase tracking-[0.3em] hover:bg-amber-700 transition-all shadow-xl">Catalogar Livro</button>
            </form>
          </div>
        </div>
      )}
