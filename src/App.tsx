...
const countryFlags: Record<string, string> = {
  ...
  'japao': '🇯🇵', 'china': '🇨🇳', 'coreia do sul': '🇰🇷', 'australia': '🇦🇺', 'timor leste': '🇹🇱'
};
...
export default function App() {
  ...
  async function startReadingFromShuffle() {
  ...
  const analytics = useMemo(() => {
    ...
    const counters = { authors: {} as Record<string, number>, countries: {} as Record<string, number>, formats: {} as Record<string, number>, monthly: Array(12).fill(0) };
        ...
        {currentView === 'library' ? (
          <>
            <div className="bg-white p-2 rounded-[1.5rem] border border-slate-200 flex flex-col lg:flex-row gap-2 shadow-sm">
              ...
              <div className="flex gap-2 p-1">
                ...
                <button onClick={handleShuffle} className="p-3 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm group">
                  <Shuffle size={18} className="group-hover:rotate-12 transition-transform" />
                </button>
              ...
              {books.filter(b => (b.title.toLowerCase().includes(searchTerm.toLowerCase()) || b.author.toLowerCase().includes(searchTerm.toLowerCase())) && (filterStatus === 'Todos' || b.status === filterStatus)).map(book => (
                <div key={book.id} className="bg-white p-5 rounded-[2rem] border border-slate-100 flex gap-6 relative group shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="flex-1 py-1 min-w-0">
                    ...
                    <div className="flex gap-2 mt-4"><span className={`text-[10px] font-black px-2 py-1 rounded uppercase ${book.status === 'Concluído' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>{book.status}</span></div>
                  </div>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
             <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-6 rounded-3xl border shadow-sm"><User className="text-orange-600 mb-2"/><p className="text-xs font-bold text-slate-400 uppercase mb-1">Top Autor</p><p className="text-xl font-black text-slate-900 truncate">{analytics.topAuthors[0]?.[0] || '-'}</p></div>
              <div className="bg-white p-6 rounded-3xl border shadow-sm"><Globe className="text-purple-600 mb-2"/><p className="text-xs font-bold text-slate-400 uppercase mb-1">Top País</p><p className="text-xl font-black text-slate-900 uppercase">{analytics.topCountries[0]?.[0] || '-'}</p></div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm"><h2 className="text-sm font-black uppercase text-slate-900 mb-8 flex items-center gap-2"><Sparkles className="w-4 h-4 text-blue-600"/> Evolução Mensal</h2><div className="flex items-end justify-between h-32 gap-2">{analytics.mensal.map((count, i) => (<div key={i} className="flex-1 flex flex-col items-center gap-2"><div className="w-full bg-blue-100 rounded-t-lg transition-all hover:bg-blue-600 relative group" style={{ height: `${(count / (Math.max(...analytics.mensal) || 1)) * 100}%`, minHeight: '4px' }}></div><span className="text-[9px] font-bold text-slate-400 uppercase">{['J','F','M','A','M','J','J','A','S','O','N','D'][i]}</span></div>))}</div></div>
            </div>
          </div>
        )}
      </main>

      {/* 🎲 MODAL DE SORTEIO VISUAL PREMIUM */}
      {isShuffleOpen && shuffledBook && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 shadow-2xl text-center relative overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <button onClick={() => setIsShuffleOpen(false)} className="absolute top-6 right-6 p-2 bg-slate-100 rounded-full hover:bg-red-50 hover:text-red-500 transition-all z-20"><X size={20}/></button>
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="w-40 h-60 bg-slate-50 rounded-[2rem] mx-auto shadow-2xl overflow-hidden border-4 border-white transform hover:scale-105 transition-transform duration-500">
                {shuffledBook.cover_url ? <img src={shuffledBook.cover_url} className="w-full h-full object-cover" alt={shuffledBook.title}/> : <div className="w-full h-full flex items-center justify-center"><BookMarked size={40} className="text-slate-200"/></div>}
              </div>

              <div>
                <h3 className="text-xl font-black text-slate-900 leading-tight mb-2 px-2">{shuffledBook.title}</h3>
                <p className="text-sm font-bold text-slate-400">{shuffledBook.author}</p>
              </div>

              <button onClick={startReadingFromShuffle} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-blue-100 hover:bg-slate-900 hover:shadow-none transition-all transform active:scale-95">Começar Leitura Agora</button>
              <button onClick={handleShuffle} className="text-xs font-bold text-slate-400 hover:text-blue-600 transition-colors uppercase tracking-widest">Sortear outro</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] p-8 max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-black text-slate-900">{editingBookId ? 'Editar Registro' : 'Novo Registro'}</h2><button onClick={() => setIsModalOpen(false)} className="p-2 bg-slate-50 rounded-full hover:bg-slate-100 transition-colors"><X/></button></div>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1"><label className="text-xs font-bold text-slate-400 ml-1 flex items-center gap-1"><Tag size={12}/> Gênero Literário</label><select className="w-full bg-slate-50 rounded-2xl px-5 py-4 text-sm font-bold outline-none appearance-none cursor-pointer" value={formData.genre} onChange={e => setFormData({...formData, genre: e.target.value})}><optgroup label="Gestão & Negócios"><option>Gestão</option><option>Estratégia</option><option>Liderança</option><option>Vendas</option></optgroup><optgroup label="Técnico & Acadêmico"><option>Tecnologia</option><option>História</option></optgroup><optgroup label="Literatura"><option>Ficção</option><option>Romance</option><option>Fantasia</option></optgroup><option>Outros</option></select></div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1"><label className="text-xs font-bold text-slate-400 ml-1 flex items-center gap-1"><PlayCircle size={12}/> Data Início</label><input type="date" className="w-full bg-slate-50 rounded-2xl px-5 py-4 text-sm font-bold outline-none" value={formData.started_at} onChange={e => setFormData({...formData, started_at: e.target.value})}/></div>
                <div className="space-y-1"><label className="text-xs font-bold text-slate-400 ml-1 flex items-center gap-1"><StopCircle size={12}/> Data Fim</label><input type="date" className="w-full bg-slate-50 rounded-2xl px-5 py-4 text-sm font-bold outline-none" value={formData.finished_at} onChange={e => setFormData({...formData, finished_at: e.target.value})}/></div>
              </div>
              <div className="grid grid-cols-2 gap-4"><select className="bg-slate-50 rounded-2xl px-5 py-4 text-sm font-bold outline-none appearance-none" value={formData.format} onChange={e => setFormData({...formData, format: e.target.value})}><option>Físico</option><option>E-book</option><option>Audiobook</option></select><select className="bg-slate-50 rounded-2xl px-5 py-4 text-sm font-bold outline-none appearance-none" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as BookStatus})}><option value="Na Fila">Na Fila</option><option value="Lendo">Lendo</option><option value="Concluído">Concluído</option><option value="Abandonado">Abandonado</option></select></div>
